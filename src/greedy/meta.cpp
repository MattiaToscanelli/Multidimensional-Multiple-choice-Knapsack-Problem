...
int main(int argc, char *argv[]) {
    ...
    double temperatura = 2000000.0;
    int final = 0;
    std::vector<int> bestSolution(instance.nclasses);

    if (feasible) {
        for (int i = 0; i < instance.nclasses; i++) {
            bestSolution[i] = instance.solution[i];
        }
        final = value;
    }
            ...
            for (int j = 0; j < instance.nitems[i]; j++) {
                ...
                if (!checkIfWeightIsExceeded(&instance.weights[i][weightSum[i][j]->index * instance.nresources],
                    ...
                    if (instance.values[i][weightSum[i][j]->index] > instance.values[i][instance.solution[i]] || !feasible) {
                        ...
                        if (value > final) {
                            final = value;
                            for (int k = 0; k < instance.nclasses; k++) {
                                bestSolution[k] = instance.solution[k];
                            }
                        }
                        instance.solution[i] = weightSum[i][j]->index;
                        changedItems = true;
                    ...
                    if (instance.values[i][weightSum[i][j]->index] - instance.values[i][instance.solution[i]] != 0 &&
                        (exp((instance.values[i][weightSum[i][j]->index] - instance.values[i][instance.solution[i]]) / temperatura) > (double) rand() / RAND_MAX))
                    {
                        value -= instance.values[i][instance.solution[i]];
                        value += instance.values[i][weightSum[i][j]->index];
                        instance.solution[i] = weightSum[i][j]->index;
    ...
    if (feasible) {
        for (int i = 0; i < instance.nclasses; i++) {
            instance.solution[i] = bestSolution[i];
        }
        value = final;
    }
